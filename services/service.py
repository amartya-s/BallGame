...
from BallGame.actors.player import Player
from BallGame.actors.bullet import Bullet
from BallGame.actors.ball import Ball
...
class GameService(object):
    ...
    def __init__(self, game, score_var):
        ...
        self.player = None
        self.ball_diameter = Ball.BALL_DIAMETER
    ...
    def move_player(self, event):
        ...
        for line in GameService.LINES:
            self.canvas.delete(line)

        # line1 = self.canvas.create_line(self.player.player_surrounding_rectangle_coords[0],self.player.player_surrounding_rectangle_coords[1],self.player.player_surrounding_rectangle_coords[0],0, fill='white')
        # line2 =self.canvas.create_line(self.player.player_surrounding_rectangle_coords[2],self.player.player_surrounding_rectangle_coords[3],self.player.player_surrounding_rectangle_coords[2],0, fill='white')
        #
        # GameService.LINES.extend([line1, line2])
    ...
    def collision_with_player(self, ball):
        ...
        if not ball.in_collision:
            # top on collision
            if py1 <= by2 and (px1 <= bx2 and px2 >= bx1):
                direction = Direction.UP
                angle = -angle
                print(direction)

            # left collision
            if px1 <= bx2 < px2 and (py1 < by2 and py2 > by1):
                print(self.player.player_surrounding_rectangle_coords)
                print(self.canvas.coords(ball.canvas_index))
                print(py1 < by2 and py2 > by1)
                angle = - angle
                print("left")

            # right collision
            if px2 >= bx1 > px1 and (py1 < by2 and py2 > by1):
                angle = - angle
                print("right")

            # bottom collision
            if py2 <= by1 and (px1 <= bx2 and px2 >= bx1):
        ...
        ball.angle_wrt_x_axis = angle
        ball.direction = direction

        return ball.in_collision
